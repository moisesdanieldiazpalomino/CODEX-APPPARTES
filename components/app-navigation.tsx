...
import { ClipboardList, Gauge, MoreHorizontal, UserCog, UsersRound, Wrench } from "lucide-react";
...
const technicianLinks = [{ href: "/dashboard", label: "Mis trabajos", icon: ClipboardList }];
...
export function AppNavigation({ role, variant }: { role: UserRole; variant: "desktop" | "mobile" }) {
  ...
  return role === "OFFICE" ? (
        <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-4 border-t border-[#d2e2e2] bg-white px-2 pb-[max(0.45rem,env(safe-area-inset-bottom))] pt-2 shadow-[0_-10px_30px_rgba(18,51,61,0.08)] lg:hidden" aria-label="Navegación móvil">
          ...
          <details className="group relative">
            <summary className={`flex min-h-12 cursor-pointer list-none flex-col items-center justify-center gap-0.5 rounded-lg text-[0.7rem] font-semibold [&::-webkit-details-marker]:hidden ${isCurrent("/propuestas") || isCurrent("/usuarios") ? "bg-[#e6f4f3] text-[#075f65]" : "text-[#58717a]"}`}>
