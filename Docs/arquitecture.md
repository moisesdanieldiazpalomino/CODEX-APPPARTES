...
`app/globals.css` define los colores, la tipografía y las superficies compartidas. La identidad usa azul petróleo, turquesa y tonos de niebla; el ámbar se reserva para trabajos pendientes. `components/app-shell.tsx` organiza el encabezado y la sesión, mientras `components/app-navigation.tsx` marca la sección activa. La oficina ve Trabajos, Clientes y máquinas, Calculadora, Propuestas y Usuarios en el lateral; en móvil, Trabajos y Clientes quedan visibles y las otras áreas van en «Más». El técnico conserva «Mis trabajos» y accede a Calculadora tanto en el lateral como en la barra móvil. «Resumen» no aparece como opción para ningún perfil; `/dashboard` sigue siendo la página tras entrar y el logotipo permite volver a ella. El resumen prioriza la agenda de hoy sobre los indicadores. Las pantallas de acceso y configuración comparten una composición adaptable; los formularios y fichas conservan componentes reutilizables de `components/ui/`. Al añadir una pantalla, respeta esta jerarquía, los estados visibles y las áreas táctiles.

`/calculadora` vive bajo el layout protegido de `app/(app)/`. `components/calculator.tsx` gestiona únicamente el formulario y el resultado en el navegador; `lib/calculator.ts` contiene el cálculo puro, validación de entradas y formato. No usa Supabase, no guarda historial y no añade operaciones de negocio.
